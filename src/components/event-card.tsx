    ...
    <Card className="overflow-hidden hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 border-green-100/50 dark:border-green-800/50 bg-gradient-to-b from-white to-green-50/30 dark:from-background dark:to-green-950/20">
      {!isUpcoming && event?.images && (
        <div className="relative h-48 w-full overflow-hidden">
          <Image src={event.images[0] || "/placeholder.svg"} alt={event.title} fill className="object-cover transition-transform duration-300 hover:scale-105" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
        </div>
      )}
      <CardContent className="p-6">
        <h3 className="text-xl font-semibold mb-4 text-green-700 dark:text-green-300">{event?.title}</h3>
        <div className="space-y-3 mb-4">
          <div className="flex items-start group">
            ...
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{event?.date}</span>
          </div>
          {event?.time && (
            <div className="flex items-start group">
              <div className="p-1.5 rounded-full bg-green-100 dark:bg-green-900/40 mr-3 group-hover:bg-green-200 dark:group-hover:bg-green-800/60 transition-colors">
      ...
      {isUpcoming && event?.registrationLink && (
        <CardFooter className="p-6 pt-0">
          <Button className="w-full bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-600 text-white transition-colors duration-200 shadow-md hover:shadow-lg" asChild>
