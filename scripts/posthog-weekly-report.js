...
/**
 * PostHog Weekly Analytics Report Generator
 * Fetches analytics data from PostHog API for the last 7 days and sends via email
 *
...
async function getWeeklyAnalytics() {
  try {
    console.log("Fetching weekly analytics from PostHog...\n");
    ...
    console.log("Weekly analytics fetched successfully.");
...
<head>
	<meta charset="UTF-8">
	<title>DK24 Weekly Analytics Report</title>
	<style>
		* {
...
<body>
	<div class="container">
		...
		<div class="header">
			<h2 style="margin: 0; font-size: 1.5em; font-weight: 600;">Weekly Analytics Report</h2>
			<p>${analytics.weekStart} to ${analytics.weekEnd}</p>
      ...
      parseFloat(analytics.engagementRate) > 0
		...
		${
      analytics.formSubmissions > 10
        ? `
		<div class="success">
			<strong>Great Week!</strong> ${analytics.formSubmissions} new community members joined this week. Keep building together!
		</div>
		`
        : analytics.formSubmissions > 3
          ? `
		<div class="info">
			<strong>Good Progress!</strong> ${analytics.formSubmissions} new members joined this week.
		</div>
		`
          : ""
    }
      ...
      analytics.errors > 0
        ? `
		<div class="alert">
			<strong>Attention Required:</strong> ${analytics.errors} error${analytics.errors > 1 ? "s" : ""} detected this week.
			${
        ...
        : `
		<div class="success">
			<strong>No Errors Detected!</strong> The website ran smoothly this week.
		</div>
		`
    }
		...
		<div class="footer">
			...
			<p style="font-size: 0.8em; color: #a3a3a3; margin: 15px 0 5px 0;">
				Automated weekly report from <strong style="color: #0a0a0a;">DK24</strong><br>
...
async function main() {
  try {
    console.log("[DK24] Starting weekly report generation...\n");

    const analytics = await getWeeklyAnalytics();
    const emailHTML = generateEmailHTML(analytics);

    const subject = `DK24 Weekly Report - ${analytics.uniqueVisitors.toLocaleString()} Visitors, ${analytics.formSubmissions} New Members`;
    ...
    console.log("\n[COMPLETE] Weekly report generation complete!\n");
  } catch (error) {
    console.error("[ERROR] Failed to generate weekly report:", error);
    process.exit(1);
  }
}
